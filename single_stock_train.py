...
image_dir = r'D:/FuXuan/Coding/单指数/jd/images_T'
label_dir = r'D:/FuXuan/Coding/单指数/jd/labels_T'
...
save_path = r'D:/FuXuan/Coding/单指数/国债期货_T单指数训练训练.pth'
...
# 用于输出预测结果
image_dir_pred = r'D:/FuXuan/Coding/单指数/jd/images_T_pred'
dataset_pred = CustomDataset(image_dir_pred,label_dir,transform)

# Assuming you have a data loader
data_loader = DataLoader(dataset_pred, batch_size=50, shuffle=False)
# Set the model to evaluation mode
model.eval()

dates = []
file_names = os.listdir(image_dir_pred)
for file_name in file_names:
    dates.append(file_name.split('_')[1].split('.')[0])
...
output_to_use = pred.numpy()
output_to_use_df = pd.DataFrame(output_to_use,index = dates,columns=['signal'])
print(output_to_use_df)
output_to_use_df.to_csv(r'D:/FuXuan/Coding/单指数/jd/T_Pred_output.csv')