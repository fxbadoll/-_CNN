...
def single_stock_output_pic(open_i,close_i,high_i,low_i,volume_i):
    # -*- coding: utf-8 -*-
    """
...
# 用5天的数据图来看后5天的涨跌，这个时间参数用time_gap来表示
    ...
    time_gap2 = 5 #向后预测多长时间的涨跌
    moving_avg = 20 #用多长时间的移动平均值
    # 选择用哪个单个指数来进行训练

    single_stock = pd.concat([open_i,close_i,high_i,low_i,volume_i],axis=1)
...
# 将数据保存为csv文件
    #single_stock.to_csv("D:\FuXuan\Coding\单指数\国债期货\数据整理核对_2.csv")
...
# 遍历数据，画图，并保存图片和标签
    ...
    # 设置背景色
        ...
        plt.savefig(f'D:\\FuXuan\\Coding\\单指数\\jd\\images_T\\image{i:06d}_{start_date}.png',dpi=500)
        plt.close()  
    
        with open(f'D:\\FuXuan\\Coding\\单指数\\jd\\labels_T\image{i:06d}_{start_date}.txt', 'w') as file:  
    # 将数字写入文件  
...
if __name__ == '__main__':
    ...
    input_path = 'D:\FuXuan\Coding\多个股_CNN\个股行情数据'
    output_path = 'D:\FuXuan\Coding\多个股_CNN\jd'

    
# 确定股票代码

    open_ = pd.read_csv(input_path + '\开盘价.csv',index_col=0)
    close_ = pd.read_csv(input_path + '\收盘价.csv',index_col=0)
    high_ = pd.read_csv(input_path + '\最高价.csv',index_col=0)
    low_ = pd.read_csv(input_path + '\最低价.csv',index_col=0)
    volume_ = pd.read_csv(input_path + '\成交量.csv',index_col=0)
